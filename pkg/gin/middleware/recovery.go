	...
	return func(c *gin.Context) {
		defer func() {
			if err := recover(); err != nil {
				...
				// 链接中断的情况
				if brokenPipe {
					logger.ErrorField(c.Request.URL.Path,
						zap.Time("time", time.Now()),
						zap.Any("error", err),
						zap.String("http", string(httpRequest)),
					)
					_ = c.Error(err.(error))
					c.Abort()
					// 链接已断开，无法写状态码
				...
				// 如果不是链接中断，就开始记录堆栈信息
				logger.ErrorField("recovery from panic",
					zap.Time("time", time.Now()),            // 记录时间
					zap.Any("error", err),                   // 记录错误信息
					zap.String("http", string(httpRequest)), // 请求信息
					zap.Stack("stacktrace"),                 // 调用堆栈信息
				)

				// 返回 500 状态码
				response.Abort500(c, err.(error).Error())
			}
		}()
		c.Next()
	}
