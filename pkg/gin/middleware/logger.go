...
func Logger() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if responseStatus > 400 && responseStatus <= 499 {
			...
			logger.WarnField("HTTP Warning"+cast.ToString(responseStatus), logFields...)
		} else if responseStatus >= 500 && responseStatus <= 599 {
			// 除了内部错误，记录 error
			logger.WarnField("HTTP Error"+cast.ToString(responseStatus), logFields...)
		} else {
			logger.DebugField("HTTP Access Log", logFields...)
		}
	}
}
